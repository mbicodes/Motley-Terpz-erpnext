...
def send_daily_report():
    ...
    try:
        ...
        frappe.sendmail(
            ...
            subject=f"Daily Operations Report — {date_label}",
            message=html,
            delayed=False,
        )
...
<style>
  * {{ margin:0; padding:0; box-sizing:border-box; }}
  ...
  .wrap {{ max-width:780px; margin:0 auto; background:#f1f5f9; padding:24px 16px 40px; }}

  /* ── Header ── */
  .hdr {{ background:linear-gradient(135deg,#0f172a 0%,#1e293b 100%);
  ...
  .hdr-brand span {{ color:#38bdf8; }}
  .hdr-date {{ background:rgba(255,255,255,.1); border-radius:8px;
               padding:6px 14px; color:rgba(255,255,255,.8); font-size:12px;
               font-weight:600; white-space:nowrap; }}
  .hdr-title {{ margin-top:14px; color:rgba(255,255,255,.6); font-size:13px;
  ...
  .kpi-lbl {{ font-size:10px; font-weight:700; text-transform:uppercase;
              letter-spacing:.5px; color:#64748b; margin-bottom:4px; }}
...
<div class="wrap">
  ...
  <div class="hdr">
    <div class="hdr-top">
      <div class="hdr-brand">Motley <span>&amp;</span> TSBC</div>
      <div class="hdr-date">{date_label}</div>
    </div>
    <div class="hdr-title">Daily Operations Report</div>
  </div>
...
{_tomorrow_section(tm_rows, tom_label)}
  ...
  <div class="footer">
    Auto-generated by ERPNext · {datetime.now().strftime("%Y-%m-%d %H:%M")} UTC ·
    <a href="https://stage.alltechvirtual.com">stage.alltechvirtual.com</a>
  </div>
