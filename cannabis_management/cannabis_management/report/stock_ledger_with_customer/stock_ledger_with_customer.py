...
# voucher_type -> child table that carries the item rate, the parent field that holds the party,
# whether that party is a customer or supplier, and which output column the rate belongs in
VOUCHER_ITEM_MAP = {
	"Sales Invoice": {
...
def execute(filters=None):
	...
	update_voucher_party_and_rate(data)
	update_included_uom_in_report(columns, data, include_uom, conversion_factors)
	return columns, data


def update_voucher_party_and_rate(data):
	"""Populate 'customer'/'supplier' and 'rate'/'purchase_rate' columns per row based on the
	row's voucher_type: rate comes from that voucher's item child table, party from its parent doc."""

	vouchers_by_type = defaultdict(set)
	items_by_type = defaultdict(set)

	for row in data:
		voucher_type = row.get("voucher_type")
		voucher_no = row.get("voucher_no")
		if not voucher_type or not voucher_no or voucher_type not in VOUCHER_ITEM_MAP:
			continue

		vouchers_by_type[voucher_type].add(voucher_no)
		items_by_type[voucher_type].add(row.get("item_code"))

	rate_map = {}
	party_map = {}

	for voucher_type, voucher_nos in vouchers_by_type.items():
		config = VOUCHER_ITEM_MAP[voucher_type]

		child_rows = frappe.get_all(
			config["child_doctype"],
			filters={
				"parent": ("in", list(voucher_nos)),
				"item_code": ("in", list(items_by_type[voucher_type])),
			},
			fields=["parent", "item_code", f"{config['rate_field']} as rate"],
		)
		for child_row in child_rows:
			rate_map[(voucher_type, child_row.parent, child_row.item_code)] = child_row.rate

		if config["party_field"]:
			parent_rows = frappe.get_all(
				voucher_type,
				filters={"name": ("in", list(voucher_nos))},
				fields=["name", f"{config['party_field']} as party"],
			)
			for parent_row in parent_rows:
				party_map[(voucher_type, parent_row.name)] = parent_row.party

	for row in data:
		voucher_type = row.get("voucher_type")
		voucher_no = row.get("voucher_no")
		config = VOUCHER_ITEM_MAP.get(voucher_type)
		...
		rate = rate_map.get((voucher_type, voucher_no, row.get("item_code")))
		row[config["rate_column"]] = rate

		if config["party_type"] == "customer":
			row["customer"] = party_map.get((voucher_type, voucher_no))
		elif config["party_type"] == "supplier":
			row["supplier"] = party_map.get((voucher_type, voucher_no))
