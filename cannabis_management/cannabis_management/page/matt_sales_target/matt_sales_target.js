frappe.pages['matt-sales-target'].on_page_load = function (wrapper) {
    var page = frappe.ui.make_app_page({
        parent: wrapper,
        title: 'Sales Target - Matt',
        single_column: true
    });

    $(wrapper).find('.layout-main-section').html(getMattDashboardHTML());
    ...
    var showLegacyAR = todayStr >= LEGACY_AR_START;
    ...
    // ── AR Matrix renderer ────────────────────────────────────────
    // Legacy AR rows only show data from June 1 2026 onwards.
    // Before that date they render as N/A.
    function renderArMatrix(containerId, ar, cols) {
        ...
        function naTd() {
            return '<td class="sd-matrix-num" style="color:#94a3b8;font-style:italic;">N/A</td>';
        }
    ...
    function downloadCSV() {
        ...
        html += '<tr><td class="title" colspan="' + totalCols + '">Sales Target - Matt &mdash; ' + he(periodLabel) + ' View</td></tr>';
        ...
        a.href   = url;
        a.download = 'Sales_Target_Matt_' + periodLabel + '_' + new Date().toISOString().slice(0,10) + '.xls';
...
function getMattDashboardHTML() {
    return `
    <div class="sd-dash">
      <div class="sd-header">
        <div class="sd-header-left">
          <div class="sd-label">Motley Terpz</div>
          <h1>Sales Dashboard <span style="font-size:16px;font-weight:500;color:#64748b;">— Matt</span></h1>
        ...
        <div class="sd-header-right">
          ...
          <button class="sd-refresh-btn" onclick="window.mattSalesDash && window.mattSalesDash.reload()">
      ...
      <div class="sd-matrix-bar">
        ...
        <div class="sd-matrix-controls">
          ...
          <button class="sd-export-btn" onclick="window.mattSalesDash && window.mattSalesDash.exportExcel()" title="Export to Excel">
