...
function poBindEvents() {
	...
	$(document).on("click", ".po-download-preorder", function (e) {
		e.stopPropagation();
		poDownloadPdf($(this).data("name"));
	});
	...
	$(document).on("click", ".po-submit-preorder", function (e) {
		e.stopPropagation();
		poSubmitPreorder($(this).data("name"));
	});
...
function poDownloadPdf(name) {
	...
	frappe.show_alert({
		message: "Downloading <b>" + frappe.utils.escape_html(name) + ".pdf</b>",
		indicator: "blue",
	}, 10);
}
...
function poSubmitPreorder(name) {
	frappe.confirm(
		"Submit <b>" + frappe.utils.escape_html(name) + "</b>?<br><br>" +
		"A submitted preorder can no longer be edited.",
		function () {
			frappe.call({
				...
				callback: function (r) {
					...
					frappe.show_alert({
						message: "Preorder <b>" + frappe.utils.escape_html(name) + "</b> submitted",
						indicator: "green",
					});
					poLoadRecent();
				},
			...
			if (r.message) {
				...
				frappe.show_alert({
					message:
						...
						'<a href="' + poPdfUrl(name) + '" download="' + frappe.utils.escape_html(name) + '.pdf">download PDF</a>',
					indicator: "green",
				}, 10);
				poClear();
				poLoadRecent();
