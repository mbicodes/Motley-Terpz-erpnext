...
def get_payable_summary(
    ...
    Filtes ONLY for Party Type = Supplier
    """
    ...
    # We add "party_type": "Supplier" to filters if supported, 
    # OR we filter the results if the report doesn't support implicit single party type.
    # Accounts Payable Summary usually supports basic filters.
    # However, to be absolutely sure we only get Suppliers, we will filter the output as well.
    filters = frappe._dict(
        {
            ...
            "show_gl_balance": 0,
            "show_sales_person": 0,
            "party_type": "Supplier" # explicit filter passed to report
        }
    )
    ...
    result = []
    for row in data:
        # Extra safety check: ensure we only return Suppliers
