frappe.pages['sales-overview'].on_page_load = function (wrapper) {
		...
		<div class="svo-container">
			<div class="svo-header">
				<div class="svo-header-left">
					...
					<p class="svo-subtitle" id="svo-subtitle">All sales &mdash; invoices, orders, delivery notes &amp; payments received</p>
				</div>
	...
	function apply_preset(preset) {
		...
		state.to_date = to;
		page.main.find('#svo-from').val(from);
		page.main.find('#svo-to').val(to);
	}

	// ── Data loading ──────────────────────────────────────────────────────
	function load_data() {
		page.main.find('#svo-data-area').html(`
	...
	function render() {
		...
		page.main.find('#svo-subtitle').text(
			who + ' — invoices, orders, delivery notes & payments received');
		...
		page.main.find('#svo-tabs .svo-tab').on('click', function () {
	...
	function render_charts() {
		...
		var t = res.trend;
		if (t.labels.length) {
			new frappe.Chart('#svo-trend-chart', {
				type: 'bar',
				height: 260,
			...
			$('#svo-trend-chart').html('<div class="svo-empty">No data in this period</div>');
		}

		var p = res.kpis.payments;
		if (p.total > 0) {
			new frappe.Chart('#svo-split-chart', {
				type: 'donut',
				height: 260,
			...
			$('#svo-split-chart').html('<div class="svo-empty">No payments in this period</div>');
		}
	}
	...
	function set_tab(tab) {
		...
		page.main.find('#svo-tabs .svo-tab').removeClass('svo-tab-active');
		page.main.find(`#svo-tabs .svo-tab[data-tab="${tab}"]`).addClass('svo-tab-active');
		...
		if (!rows.length) {
			page.main.find('#svo-list-area').html('<div class="svo-empty">No records in this period</div>');
			return;
		}
		...
		page.main.find('#svo-list-area').html(`
	...
	frappe.call({
		...
		callback: function (r) {
			...
			var toggle = page.main.find('#svo-sp-toggle');
			...
			var sel = page.main.find('#svo-company');
	...
	// ── Filter events ─────────────────────────────────────────────────────
	page.main.find('#svo-company').on('change', function () {
		state.company = $(this).val();
		load_data();
	});
	page.main.find('#svo-preset').on('change', function () {
		apply_preset($(this).val());
		load_data();
	});
	page.main.find('#svo-from, #svo-to').on('change', function () {
		page.main.find('#svo-preset').val('custom');
		state.from_date = page.main.find('#svo-from').val();
		state.to_date = page.main.find('#svo-to').val();
		load_data();
	});
	page.main.find('#svo-refresh').on('click', load_data);
};
