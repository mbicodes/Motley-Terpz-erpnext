...
class NikkiExpenseEntry(Document):
    ...
    def create_payment_entry(self):
        try:
            ...
            pe.mode_of_payment = self.mode_of_payment or None
            ...
            # Party mapping
            pe.party_type = self.party_type or None
            if self.party_type == "Customer":
                pe.party = self.customer or None
            elif self.party_type == "Supplier":
                pe.party = self.supplier or None

            if pe.party_type and not pe.party:
                frappe.throw(
                    f"Please select a {'Customer' if pe.party_type == 'Customer' else 'Supplier'} "
                    f"before saving."
                )
