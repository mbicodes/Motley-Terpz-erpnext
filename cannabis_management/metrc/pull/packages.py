...
def upsert_packages(rows, license_number):
    ...
    company = config.company_for_license(license_number)
    ...
    for pkg in rows:
        ...
        if not label:
            continue
        try:
            _upsert_tag(pkg, label, license_number, warehouse, company)
            _upsert_batch(pkg, label, license_number, warehouse, company)
        except Exception:
...
def _upsert_tag(pkg, label, license_number, warehouse, company=None):
...
def _upsert_batch(pkg, label, license_number, warehouse, company=None):
